...
class PublicacionModel3 extends DataBase
{
    ...
    {
        try {
            $query = $this->bd->insertInto('publicacion3')->values($params);
            $res = $query->execute();
            return $res;
    ...
    {
        try {
            $query = $this->bd->update('publicacion3')->set($params)->where('idpub', $idpub);
            $res = $query->execute();
            return $res;
        } catch (\PDOException $e) {
            die($e->getMessage());
    ...
    {
        try {
            $query = $this->bd->update('publicacion3')->set(array('visible' => $estado))->where('idpub', $idpub);
            $res = $query->execute();
            return $res;
        } catch (\PDOException $e) {
            die($e->getMessage());
    ...
    {
        try {
            $query = $this->bd->deleteFrom('publicacion3')->where('idpub', $idpub);
            $res = $query->execute();
            return $res;
        } catch (\PDOException $e) {
            die($e->getMessage());
    ...
    public function listarPublicaciones($init, $categoria = '%')
    {
        try {
            $query = $this->bd->from('publicacion3 pb')
                ...
                ->select(null)
                ->select('pb.idpub, pb.idcatg, ct.nombre as categoria3, pb.titulo, pb.tagname, pb.portada, pb.fecpub, pb.visible')
    ...
    {
        try {
            $query = $this->bd->from('publicacion3')->where('idpub', [$idpub])->fetch();
            if (is_array($query)) {
                return $query;
            }
            return null;
    ...
    {
        try {
            $query = $this->bd->from('publicacion3')->where('tagname', [$tagname])->fetch();
            if (is_array($query)) {
                return $query;
            }
            return null;
    ...
    public function totalPublicaciones($categoria = '%', $isWeb = false)
    {
        try {
            if ($isWeb) {
                $query = $this->bd->from('publicacion3')->select(null)->select('COUNT(*) as total')->where("idcatg LIKE '{$categoria}' AND fecpub <= NOW() AND visible = 'S'")->fetch('total');
            } else {
                $query = $this->bd->from('publicacion3')->select(null)->select('COUNT(*) as total')->where("idcatg LIKE '{$categoria}'")->fetch('total');
            }
            return $query;
        } catch (\PDOException $e) {
            die($e->getMessage());
    ...
    public function paginationAdmin($cateId, $total, $pag)
    {
        ...
        if ($pag == 1) {
            ...
            $list = '<li class="page-item"><a class="page-link" href="/admin/publicacion3/' . $cateId . '/' . ($pag - 1) . '"><span aria-hidden="true">&laquo;</span></a></li>';
        }
        for ($i = 1; $i <= $numPag; $i++) :
            $active = $i == $pag ? 'active' : '';
            $list .= '<li class="page-item ' . $active . '"><a class="page-link" href="/admin/publicacion3/' . $cateId . '/' . $i . '">' . $i . '</a></li>';
        endfor;
        if ($pag == $numPag) {
            $list .= '<li class="page-item disabled"><a class="page-link" href="#"><span aria-hidden="true">&raquo;</span></a></li>';
        } else {
            $list .= '<li class="page-item"><a class="page-link" href="/admin/publicacion3/' . $cateId . '/' . ($pag + 1) . '"><span aria-hidden="true">&raquo;</span></a></li>';
        }
        return $list;
    }
